...
В файле [./numpy_CNN/numpy_resnet.py](./numpy_CNN/numpy_resnet.py) реализация resnet-101 на numpy. Там находятся:
...
В [./numpy_CNN/numpy_resnet_mnist.ipynb](./numpy_CNN/numpy_resnet_mnist.ipynb) произведено обучение на датасете MNIST моей реализации resnet-101 на numpy. Результаты обучения resnet на numpy на графике ниже. Также в этом файле определяется и обучается небольшая сверточная нейронная сеть, чтобы продемонстрировать, как пользоваться реализованными класами.

![numpy resnet-101 results](./images_for_readme/numpy_resnet_results.png)


Реализация свертки, основанной на матричном умножении, ускорило обучение resnet101 на MNIST более чем в 33 раза! На изоборажении ниже видно, что ранее одна эпоха треборвала более 114 часов, теперь около 3 часов. Ниже подтверждающий скриншот.
